...
7. classres 종류
8. classtype 3 -> 예외(?), 특수수업
9. 이름에 공백이 있는 교수님들(외국인 교수님들)
10. 같은 과에서는 오래된 정교수 먼저 배정하는 경우도 있다고 함(우선순위 고려)
*/	
...
int main() {
	...
	//만들어지는 textfile 의 이름은 classlist.txt
	combine_files();
	...
	//텍스트 파일의 정보들을 읽어들이고 메모리에 저장
		...
		//**cl은 Class List을 의미함. **classcnt : 강의의 개수
	...
	cl = (CL *)malloc(sizeof(CL)*(classcnt));   // 입력받은 파일 총 개수 세기 + 동적할당
		...
		//**ri는 Room Info를 의미함. **roomcnt : 강의실 개수
	...
	ri = (RI *)malloc(sizeof(RI)*(roomcnt));   // 갖고있는 파일 총 개수 세기 + 동적할당
		...
		//ri[]구조체 배열에 있는 모든 강의실의 시간표를 0으로 초기화 (쓰레기값 초기화)
	...
	for (i = 0; i < roomcnt; i++) {
		...
		//cl[]구조체 배열에 있는 모든 수업의 시간표를 0으로 초기화 (쓰레기값 초기화)
	...
	for (i = 0;i < classcnt;i++) {
		...
		//메모리에 저장한 정보들을 전부 출력(classlixt.txt에서)(디버깅용)
	printf("\n\n//ALL DATA SAVED TO MEMORY(FROM CLASSLIST.TXT)");
	printf("\n//메모리에 저장된 모든 정보들(cl구조체 배열)");
	for (i = 0;i < classcnt;i++) {
